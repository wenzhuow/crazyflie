...
def set_initial_position(scf, x, y, z, yaw_deg):
    scf.cf.param.set_value('kalman.initialX', str(x))
    scf.cf.param.set_value('kalman.initialY', str(y))
    scf.cf.param.set_value('kalman.initialZ', str(z))


    yaw_radians = math.radians(yaw_deg)
##    scf.cf.param.set_value('kalman.initial_yaw', str(yaw_radians))
...
def run_sequence(scf, sequence, base_x, base_y, base_z, yaw):
    ...
    for position in sequence:
        ...
        for i in range(30):
            cf.commander.send_setpoint(int(x), int(y), yaw,int(z))
            time.sleep(0.1)
...
##                data = logger.next()[1]
##
##                print(data['stabilizer.pitch'],"  ",data["stabilizer.roll"])
##                cf.commander.send_zdistance_setpoint(0-data['stabilizer.roll']*1.2,0-data["stabilizer.pitch"]*1.2,0,30)


####                cf.commander.send_setpoint(0-data['stabilizer.roll'],0-data["stabilizer.pitch"],0,3)
...
##                time.sleep(0.01)
##                it+=1
            cf.extpos.send_extpos(0,200,0)
##            print(cf.loc.receivedLocationPacket.__dict__)

##            for key in cf.param.toc.toc["flightmode"].keys():
##                print(key)
##                print(cf.param.toc.toc["flightmode"][key].__dict__)

                
##        it=0
##        print("stay")
##        while it<200:
##            cf.commander.send_setpoint(0,0,0,34000)
##            cf.param.set_value("flightmode.althold","True")
##            time.sleep(0.01)
##            it+=1
##        it=0
##        print("downward")
##        while it<200:
##            cf.commander.send_setpoint(0,0,0,31000)
##            cf.param.set_value("flightmode.althold","True")
##            time.sleep(0.01)
##            it+=1
